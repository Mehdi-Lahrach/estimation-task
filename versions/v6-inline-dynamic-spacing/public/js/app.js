...
(function () {
  ...
  function showPage(pageId) {
    ...
    if (pageId === 'intro' && state.condition === 'detailed') {
  ...
  function initConsent() {
    ...
    btn.addEventListener('click', () => {
      if (cb.checked) {
        showPage('intro');
        // Render detailed estimation inline once the page is visible
        if (state.condition === 'detailed') {
  ...
  function initIntro() {
    ...
    // Simple condition: render estimation inline on intro page
    if (state.condition === 'simple') {
      renderSimpleEstimation();
    }

    // Hide the "Begin estimation" button for both conditions
    // (simple has its own submit; detailed renders estimation inline via initConsent)
  ...
  function renderDetailedEstimation() {
    const content = document.getElementById('detailed-inline-estimation');
    if (!content) return;
    ...
    function positionCards() {
      ...
      if (!hasAdjustedZones) {
        ...
        const PAD_SVG = 30; // SVG-unit padding above+below card
  ...
  function renderSummary() {
    ...
    document.getElementById('summary-back').addEventListener('click', () => {
      // Return to intro page and scroll to the estimation section
      document.querySelectorAll('.task-page').forEach(p => p.classList.remove('active'));
      document.getElementById('page-intro').classList.add('active');
      state.currentPage = 'intro';
      document.querySelector('.task-container').style.maxWidth = '1400px';
      requestAnimationFrame(() => {
        if (window._positionEstCards) window._positionEstCards();
        const est = document.getElementById('detailed-inline-estimation');

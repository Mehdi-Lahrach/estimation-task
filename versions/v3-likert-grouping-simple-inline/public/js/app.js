...
(function () {
  ...
  function initIntro() {
    ...
    if (state.condition === 'simple') {
      ...
      // Detailed condition: navigate to phase-by-phase estimation
  ...
  function renderPhase(phaseIndex) {
    ...
    // Render steps grouped by estimation blocks — wrapped in visible containers
    phase.estimationBlocks.forEach((block) => {
      const blockSteps = phase.steps.filter(s => block.stepsIncluded.includes(s.id));
      const stepIds = blockSteps.map(s => s.id).join(', ');

      html += `
        <div class="estimation-group" style="border-color: ${phase.color}40;">
          <div class="estimation-group__header">
            <span class="estimation-group__icon">⏱</span>
            <span class="estimation-group__label">Steps ${stepIds} → ${block.label}</span>
          </div>
          <div class="estimation-group__steps">
      `;

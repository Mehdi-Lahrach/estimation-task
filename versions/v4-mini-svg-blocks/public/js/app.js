...
(function () {
  ...
  function initIntro() {
    ...
    // Simple condition: render estimation inline on intro page (no separate page)
    if (state.condition === 'simple') {
      renderSimpleEstimation();
      // Hide the "Begin estimation" button — replaced by inline submit
      const introBtn = document.getElementById('intro-continue');
      if (introBtn) introBtn.style.display = 'none';
    } else {
      // Detailed condition: navigate to single-page estimation with mini SVGs
      document.getElementById('intro-continue').addEventListener('click', () => {
        showPage('estimation');
        renderDetailedEstimation();
      });
    }
  }
  ...
  // ============================================================
  // CONDITION A: DETAILED (single-page with mini SVG process maps)
  // ============================================================

  function renderDetailedEstimation() {
    // Hide the phase nav and progress bar — not needed for single-page layout
    ...
    content.className = 'detailed-single-page';
    ...
    // Page header
    html += `
      <div class="page-card fade-in" style="margin-bottom: 20px;">
        <h1>Estimate the time for each part of the procedure</h1>
        <p>Below you can see the process map broken down by section. For each group of steps, please estimate how long it took participants on average. Click on any step to see hidden sub-actions.</p>
    ...
    // Render each phase → estimation blocks with mini SVGs
    let blockIdx = 0;
    ...
    PROCESS_MAP.phases.forEach((phase) => {
      // Phase header
      html += `
        <div class="detailed-phase-section" style="margin-bottom: 24px;">
          <div class="phase-header" style="background: ${phase.color}; border-radius: 10px 10px 0 0;">
      ...
      // For each estimation block: mini SVG + estimation card
      phase.estimationBlocks.forEach((block) => {
        const letter = blockLetters[blockIdx] || String(blockIdx + 1);
        const stepIds = block.stepsIncluded;
        const svgContainerId = `svg-block-${block.id}`;
        blockIdx++;

        html += `
          <div class="estimation-group estimation-group--visual" style="border-color: ${phase.color}40;">
            <div class="estimation-group__header">
              <span class="estimation-group__letter" style="background: ${phase.color};">${letter}</span>
              <span class="estimation-group__label">Steps ${stepIds.join(', ')} — ${block.label}</span>
            </div>
            <div class="estimation-group__svg-container">
              <div id="${svgContainerId}" class="block-svg-container"></div>
            </div>
        `;

        // Estimation input card
    ...
    // Render mini SVGs for each estimation block
    PROCESS_MAP.phases.forEach((phase) => {
      phase.estimationBlocks.forEach((block) => {
        if (typeof ProcessMapSVG !== 'undefined') {
          ProcessMapSVG.renderBlock(`svg-block-${block.id}`, block.stepsIncluded, phase);
        }
      });
    });
    ...
    // Submit button → validate all blocks then go to summary
  ...
  function renderSimpleEstimation() {
    // Renders the estimation UI inline on the intro page (simple condition)
    const container = document.getElementById('simple-inline-estimation');
    if (!container) return;

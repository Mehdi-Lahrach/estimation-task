...
const ProcessMapSVG = (() => {
  ...
  function adjustLayoutForZoneMinHeights(layout, zones, minHeights) {
    ...
    for (let idx = 0; idx < zones.length; idx++) {
      ...
      // Shift everything below this zone down; expand bands spanning the boundary
      for (const e of layout.elems) {
        if (e.type === 'band') {
          if (e.y > yMax) {
            e.y += delta;
          } else if (e.y + e.h > yMax) {
            e.h += delta;
          }
        } else {
          if (topY(e) > yMax) {
  ...
  function drawDecision(e) {
    ...
    s += tag('polygon', {
      ...
      fill: '#fff', stroke: phase.color, 'stroke-width': 2, filter: `url(#${idPrefix}-shd)`,
    });
    s += tag('text', {
      x: cx, y: cy + 1,

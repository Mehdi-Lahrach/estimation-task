...
app.post('/api/session/create', (req, res) => {
  const { prolific_pid, study_id, session_id: prolific_session_id, condition } = req.body;

  // Check for duplicate
  const existing = readJsonl('sessions.jsonl');
  const dup = existing.find(s => s.prolific_pid && s.prolific_pid === prolific_pid && s.completed);
  if (dup) {
    return res.json({ success: true, session_id: dup.session_id, already_complete: true });
  }
  ...
  if (incomplete) {
    return res.json({ success: true, session_id: incomplete.session_id, resumed: true });
  }
...
app.listen(PORT, () => {
  console.log(`Estimation task running on http://localhost:${PORT}`);
  console.log(`  Detailed condition: http://localhost:${PORT}/?CONDITION=detailed`);
  console.log(`  Simple condition:   http://localhost:${PORT}/?CONDITION=simple`);

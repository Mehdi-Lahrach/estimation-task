...
(function () {
  ...
  if (conditionParam === 'simple') {
    state.condition = 'simple';
  } else {
    state.condition = 'detailed';
  }
  ...
  function validatePhaseEstimates(phaseIndex) {
    const phase = PROCESS_MAP.phases[phaseIndex];
    const missing = phase.estimationBlocks.filter(block => {
      const est = state.estimates[block.id];
      if (!est) return true;
      const hasMin = est.minutes !== null && est.minutes !== undefined;
      const hasSec = est.seconds !== null && est.seconds !== undefined;
      return !hasMin && !hasSec; // need at least one
    });
  ...
  function renderSimpleCondition() {
      ...
      <div class="simple-section">
        ...
        <div class="estimation-block estimation-block--overall">
          ...
          <div class="estimation-block__prompt">
            How long do you think it took participants, on average, to complete the <strong>entire procedure</strong> from start to finish?
            <br><small>This includes reading instructions, entering personal details, assessing eligibility, entering vehicle information across 4 form pages, and submitting the application.</small>
          </div>
          <div class="estimation-block__input-row">
            <div class="estimation-block__input-group">
              <input type="number" class="estimation-block__input estimation-block__input--min"
    ...
    if (typeof ProcessMapSVG !== 'undefined') {
      ProcessMapSVG.render('svg-process-map', { interactive: false });
    }
